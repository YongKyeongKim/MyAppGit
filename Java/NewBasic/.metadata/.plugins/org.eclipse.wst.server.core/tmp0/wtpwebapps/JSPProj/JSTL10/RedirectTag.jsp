...
<c:set var="requestVar" value="리퀘스트 영역" scope="request"/>  
 <!-- 자바코드로 리다이렉트 -->
 <%--
 	response.sendRedirect(request.getContextPath()+"/JSTL10/ImportedPage.jsp?user=KOSMO&pass=9999");
 --%>
 <!--  JSTL태그로 리다이렉트(같은 웹 어플 안에서) -->
 <%-- 
 //방법1
 <c:redirect url="/JSTL10/ImportedPage.jsp?user=KOSMO&pass=9999"/>
 --%>

 <c:redirect url="/JSTL10/ImportedPage.jsp">
 	<c:param name="user" value="KOSMO"/>
 	<c:param name="pass" value="8989"/>
 </c:redirect>
...
<!-- 다른 어플리케이션안에 있는 페이지로 redirect 
 ...
 -->
<%-- 
  <c:redirect url="/index.jsp" context="/Test">
 	<c:param name="user" value="KOSMO"/>
 	<c:param name="pass" value="8989"/>
 </c:redirect>
--%>
