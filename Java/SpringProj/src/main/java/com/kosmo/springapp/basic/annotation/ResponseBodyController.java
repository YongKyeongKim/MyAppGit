...
public class ResponseBodyController {
	...
	//[웹브라우저로 직접 출력하기]
	//방법1]반환타입은 void로 하고 매개변수는 서블릿 API(HttpServletResponse)사용
	//@ResponseBody어노테이션 미 사용
	/*
	...
	//방법2]반환타입 String으로 하고  @ResponseBody어노테이션 사용(서블릿 API 미사용)
	//@RequestMapping("/Annotation/ResponseBody.do")한글이 깨짐	
	@RequestMapping(value="/Annotation/ResponseBody.do",produces = "text/html; charset=UTF-8")
	@ResponseBody
	public String exec() {
		//※반환되는 문자열이 응답바디에 쓰인다
		return "<h2>반환타입:void,서블릿 API 사용:HttpServletResponse</h2>";
	}
}
