...
/*
* @Aspect어노테이션]
* 1]이 클래스는 Advice역할을 하는 클래스라는 것을 컴파일러에게 알려주는 어노테이션
*/
@Aspect
public class AdviceUsingAnnotation {
	...
	/*
	  ...
	  */
	//2]타겟 클래스의 어느 지점에 현 Advice를 삽입(위빙) 할지 포인트 컷 설정
	@Pointcut("execution(public * com.kosmo.springapp..*(..))")	
	//Pointcut를 지정할 공통 메소드 원형 정의(단, 반환타입은 무조건 void,메소드명 임의)
	public void advice() {}
	...
	public Object cross_cutting_concern(ProceedingJoinPoint point) throws Throwable{
		...
		System.out.println("대상 클래스의 핵심관점(메소드명):"+core_concern);
		...
		System.out.println(core_concern+"의 총 소요시간:"+(endTime-startTime)/1000.0+"초");
