...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Resources resources= getResources();
         /*※values폴더의 strings.xml과  drawable폴더에 있는 이미지 접근시 직접
             getString()이나 getDrawable()로 직접 접근 가능- Resources객체 불필요*/
        Log.i("com.kosmo.resource09",String.format(
                "Resources객체 미 사용:%s,Resources객체 사용:%s",
                getString(R.string.codeMessage),resources.getString(R.string.codeMessage)));
        //2]자바코드로 위젯의 속성 설정
        //android:text속성]
        //2-1]setText(int resId)사용
        textView.setText(R.string.codeMessage);
        //android:textSize속성]
        //textView.setTextSize(20);//기본이 SP
        ...
        button.setOnClickListener(v->{
             /*
                ...
                */
            int[] ages=resources.getIntArray(R.array.intArray);
            String[] mountains = resources.getStringArray(R.array.strArray);
            Toast.makeText(getApplicationContext(),
                    String.format("나이대:%s%n산:%s", Arrays.toString(ages),Arrays.toString(mountains)),Toast.LENGTH_SHORT).show();
        });
    }
}